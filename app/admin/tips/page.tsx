...
export default function AdminTipsPage() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    const { data } = await supabase.from("automation_tips").select("*").order("created_at", { ascending: false });
    setItems(data || []);
    setLoading(false);
  };
    ...
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-black text-white flex items-center gap-2">
            <Lightbulb size={20} className="text-yellow-400" /> Tips & Tricks Management
          </h1>
          <p className="text-[#9CA3AF] text-sm mt-0.5">{items.length} tip{items.length !== 1 ? "s" : ""}</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-[#9CA3AF]">
          ...
          <p>{search ? "No tips match your search" : "No tips yet. Create your first one!"}</p>
        </div>
      ) : (
          ...
          {filtered.map((item) => (
            <motion.div key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                <button onClick={() => togglePublish(item)} className="p-2 rounded-lg text-[#9CA3AF] hover:text-white hover:bg-white/5 transition-all">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 30, opacity: 0 }}
              ...
              <div className="flex items-center justify-between p-5 border-b border-white/5">
                <h2 className="text-lg font-black text-white">{editingId ? "Edit Tip" : "New Tip"}</h2>
              ...
              <form onSubmit={handleSubmit} className="p-5 space-y-4">
                <div>
                  ...
                  <input value={form.title} onChange={(e) => handleTitleChange(e.target.value)} required
                    placeholder="e.g. How to Handle Dynamic Elements in Playwright" className="w-full px-4 py-2.5 rounded-xl bg-[#1A1A1A] border border-[#2A2A2A] text-white text-sm focus:outline-none focus:border-[#00FF88]/40 transition-all" />
                </div>
                ...
                <div>
                  ...
                  <input value={form.slug} onChange={(e) => setForm((p) => ({ ...p, slug: e.target.value }))} required
                    className="w-full px-4 py-2.5 rounded-xl bg-[#1A1A1A] border border-[#2A2A2A] text-white text-sm font-mono focus:outline-none focus:border-[#00FF88]/40 transition-all" />
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-medium text-[#9CA3AF] mb-1">Category</label>
                    <select value={form.category} onChange={(e) => setForm((p) => ({ ...p, category: e.target.value as AutomationTip["category"] }))}
                  ...
                  <div>
                    ...
                    <select value={form.difficulty} onChange={(e) => setForm((p) => ({ ...p, difficulty: e.target.value as AutomationTip["difficulty"] }))}
                      className="w-full px-4 py-2.5 rounded-xl bg-[#1A1A1A] border border-[#2A2A2A] text-white text-sm focus:outline-none focus:border-[#00FF88]/40 transition-all">
                ...
                <div>
                  ...
                  <textarea value={form.excerpt} onChange={(e) => setForm((p) => ({ ...p, excerpt: e.target.value }))}
                    rows={2} placeholder="Brief description of this tip..." className="w-full px-4 py-2.5 rounded-xl bg-[#1A1A1A] border border-[#2A2A2A] text-white placeholder-[#9CA3AF] text-sm focus:outline-none focus:border-[#00FF88]/40 transition-all resize-none" />
                </div>
                ...
                <div>
                  ...
                  {tab === "editor" ? (
                    <textarea value={form.content} onChange={(e) => setForm((p) => ({ ...p, content: e.target.value }))}
                      rows={12} placeholder="## Problem&#10;&#10;## Solution&#10;&#10;```javascript&#10;// your code here&#10;```" className="w-full px-4 py-3 rounded-xl bg-[#0D0D0D] border border-[#2A2A2A] text-white placeholder-[#9CA3AF] text-sm font-mono focus:outline-none focus:border-[#00FF88]/40 transition-all resize-none" />
                  ) : (
                    <div className="w-full min-h-48 px-4 py-3 rounded-xl bg-[#0D0D0D] border border-[#2A2A2A] text-[#D1D5DB] text-sm overflow-auto">
                ...
                <div>
                  ...
                  <input value={form.tags} onChange={(e) => setForm((p) => ({ ...p, tags: e.target.value }))}
                    placeholder="playwright, selectors, automation" className="w-full px-4 py-2.5 rounded-xl bg-[#1A1A1A] border border-[#2A2A2A] text-white text-sm focus:outline-none focus:border-[#00FF88]/40 transition-all" />
                </div>

                <div className="flex items-center gap-6">
                  {([["featured", "⭐ Trending"], ["published", "✅ Published"]] as const).map(([key, label]) => (
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button type="submit" disabled={saving} className="btn-primary flex-1 justify-center gap-2">
                    {saving ? <><Loader2 size={14} className="animate-spin" /> Saving...</> : (editingId ? "Update Tip" : "Create Tip")}
                  </button>
                </div>
              </form>
            </motion.div>
