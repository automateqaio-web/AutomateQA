...
import { Search, X, Loader2, BookOpen, Clock, Eye, Star } from "lucide-react";
import { LearningContent, LEARN_CATEGORIES, DIFFICULTIES, CATEGORY_COLORS, DIFFICULTY_COLORS, Difficulty } from "@/types";
...
function TutorialCard({ item, index }: { item: LearningContent; index: number }) {
  return (
    <motion.article
    ...
    >
      <Link href={`/learn/${item.slug}`} className="group block h-full">
        <div className="glass-card-hover h-full flex flex-col overflow-hidden">
          {item.cover_image ? (
            <div className="relative aspect-video overflow-hidden rounded-t-2xl">
              ...
              {item.featured && (
                <div className="absolute top-3 right-3">
                  <span className="category-badge border bg-[#00FF88]/20 text-[#00FF88] border-[#00FF88]/30 flex items-center gap-1">
            ...
            <div className="aspect-video bg-gradient-to-br from-[#1A1A1A] to-[#111] rounded-t-2xl flex items-center justify-center relative">
              <BookOpen size={40} className="text-[#2A2A2A]" />
              <div className="absolute bottom-3 left-3 flex gap-2">
                <span className={`category-badge border ${CATEGORY_COLORS[item.category] || "bg-gray-500/20 text-gray-400 border-gray-500/30"}`}>
                  {item.category}
                </span>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {loading ? (
          ...
          <AnimatePresence mode="wait">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {items.map((item, i) => <TutorialCard key={item.id} item={item} index={i} />)}
            </div>
          </AnimatePresence>
        )}
          ...
          <div ref={loaderRef} className="py-8 flex justify-center">
            {loadingMore && (
              <div className="flex items-center gap-2 text-[#9CA3AF]">
                ...
                <span className="text-sm">Loading more tutorials...</span>
              </div>
            )}
            {!hasMore && !loadingMore && (
              <p className="text-[#9CA3AF] text-sm">All tutorials loaded 🎓</p>
            )}
          </div>
        )}
      </div>
