...
interface Comment { id: string; name: string; comment: string; created_at: string; }
...
export default function VideoInteractions({
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem(`video_liked_${videoId}`)) {
      setLiked(true);
  ...
  async function handleLike() {
    ...
    setLiked(true);
    setLikesCount((c) => c + 1);
    localStorage.setItem(`video_liked_${videoId}`, "1");
    try {
      const { data } = await supabase.rpc("increment_video_likes", { video_id_param: videoId });
  ...
  async function handleComment(e: React.FormEvent) {
    ...
    try {
      ...
      setCommentError("Failed to post. Make sure the video_comments table exists in Supabase.");
    }
    setSubmitting(false);
  }
  ...
  return (
    <div className="space-y-4 mt-6 pt-6 border-t border-white/[0.07]">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <a href={`https://www.linkedin.com/sharing/share-offsite/?url=${enc(pageUrl)}`} target="_blank" rel="noopener noreferrer"
            className="p-2.5 rounded-xl border border-white/8 bg-white/[0.03] text-[#9CA3AF] hover:border-blue-500/30 hover:text-blue-400 hover:bg-blue-500/5 transition-all" title="Share on LinkedIn">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
              <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z" />
              <rect x="2" y="9" width="4" height="12" /><circle cx="4" cy="4" r="2" />
            </svg>
          </a>
          <a href={`https://wa.me/?text=${enc(title)}%20${enc(pageUrl)}`} target="_blank" rel="noopener noreferrer"
            className="p-2.5 rounded-xl border border-white/8 bg-white/[0.03] text-[#9CA3AF] hover:border-green-500/30 hover:text-green-400 hover:bg-green-500/5 transition-all" title="Share on WhatsApp">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
      ...
      <div className="rounded-2xl border border-white/8 bg-[#0E0E0E] overflow-hidden">
        ...
        >
          ...
          <ChevronDown size={16} className={`text-[#4B5563] transition-transform duration-200 ${commentsOpen ? "rotate-180 text-[#00FF88]" : "group-hover:text-white"}`} />
        </button>
            ...
            <form onSubmit={handleComment} className="space-y-4">
              ...
              <div>
                ...
                <textarea value={commentText} onChange={(e) => setCommentText(e.target.value)}
                  placeholder="Share your thoughts on this video..." maxLength={2000} rows={3} required
              ...
              <button type="submit" disabled={submitting || !name.trim() || !commentText.trim()}
