...
const instagramContent = [
  { emoji: "😂", title: "QA Meme Reels", desc: "Bite-sized testing humour in under 60s" },
  { emoji: "📸", title: "Behind the Tests", desc: "Real screenshots from automation fails" },
  { emoji: "💬", title: "Community Polls", desc: "Selenium vs Playwright — you decide" },
];

const facebookContent = [
  { emoji: "📢", title: "Announcements", desc: "First to know about new tutorials & drops" },
  { emoji: "🤝", title: "QA Community", desc: "Discussion, tips, and job board posts" },
  { emoji: "🎥", title: "Video Shares", desc: "Full-length tutorials shared to the group" },
];
...
function InstagramChannelCard() {
  return (
    ...
    >
      <div className="relative rounded-3xl border border-pink-500/20 overflow-hidden bg-gradient-to-br from-[#111]/95 via-[#0e0e0e] to-[#0a0a0a] shadow-[0_0_60px_rgba(236,72,153,0.08)]">
        <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-pink-500/70 to-transparent" />
        <div className="absolute -top-16 left-1/4 w-72 h-48 bg-pink-500/6 rounded-full blur-3xl pointer-events-none" />
        <div className="absolute bottom-0 right-0 w-48 h-32 bg-purple-600/5 rounded-full blur-2xl pointer-events-none" />

        <div className="relative z-10 p-6 sm:p-10">
          <div className="flex flex-col sm:flex-row sm:items-center gap-6 sm:gap-10 mb-8">
            <div className="flex items-center gap-5">
              <div className="relative flex-shrink-0">
                <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full overflow-hidden ring-2 ring-pink-500/40 shadow-[0_0_32px_rgba(236,72,153,0.35)]">
                  <Image src="/logo.png" alt="AutomateQA Instagram" width={96} height={96} className="object-cover" />
                </div>
                <div className="absolute -bottom-1 -right-1 w-7 h-7 rounded-full bg-gradient-to-br from-pink-500 to-purple-600 border-2 border-[#0a0a0a] flex items-center justify-center shadow-lg shadow-pink-600/40">
                  <InstagramLogo size={12} className="text-white" />
                </div>
              </div>

              <div>
                <span className="inline-flex items-center gap-1.5 text-[10px] font-black text-pink-400 bg-pink-500/10 border border-pink-500/25 px-2.5 py-1 rounded-full uppercase tracking-widest mb-2">
                  <InstagramLogo size={9} /> Instagram
                </span>
                <h2 className="text-2xl sm:text-3xl font-black text-white leading-tight tracking-tight">AutomateQA</h2>
                <p className="text-[#6B7280] text-sm font-mono mt-0.5">@automateqa.online</p>
                <p className="text-[#9CA3AF] text-sm mt-2 leading-relaxed hidden sm:block max-w-xs">
                  QA memes &amp; Reels for testers — follow for your daily dose of automation humour.
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-6 border-t border-white/[0.06]">
            {instagramContent.map(({ emoji, title, desc }) => (
              <div key={title} className="flex items-start gap-3.5 p-4 rounded-2xl bg-white/[0.025] border border-white/[0.06] hover:border-pink-500/25 hover:bg-pink-500/[0.04] transition-all duration-300 group cursor-default">
                <span className="text-xl mt-0.5 flex-shrink-0">{emoji}</span>
                <div>
                  <div className="text-sm font-black text-white group-hover:text-pink-300 transition-colors mb-0.5">{title}</div>
                  <div className="text-xs text-[#6B7280] leading-relaxed">{desc}</div>
                </div>
              </div>
            ))}
...
function FacebookChannelCard() {
  return (
    ...
    >
      <div className="relative rounded-3xl border border-blue-500/20 overflow-hidden bg-gradient-to-br from-[#111]/95 via-[#0e0e0e] to-[#0a0a0a] shadow-[0_0_60px_rgba(59,130,246,0.08)]">
        <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-blue-500/70 to-transparent" />
        <div className="absolute -top-16 left-1/4 w-72 h-48 bg-blue-500/6 rounded-full blur-3xl pointer-events-none" />
        <div className="absolute bottom-0 right-0 w-48 h-32 bg-blue-600/5 rounded-full blur-2xl pointer-events-none" />

        <div className="relative z-10 p-6 sm:p-10">
          <div className="flex flex-col sm:flex-row sm:items-center gap-6 sm:gap-10 mb-8">
            <div className="flex items-center gap-5">
              <div className="relative flex-shrink-0">
                <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full overflow-hidden ring-2 ring-blue-500/40 shadow-[0_0_32px_rgba(59,130,246,0.35)]">
                  <Image src="/logo.png" alt="AutomateQA Facebook" width={96} height={96} className="object-cover" />
                </div>
                <div className="absolute -bottom-1 -right-1 w-7 h-7 rounded-full bg-blue-600 border-2 border-[#0a0a0a] flex items-center justify-center shadow-lg shadow-blue-600/40">
                  <FacebookLogo size={12} className="text-white" />
                </div>
              </div>

              <div>
                <span className="inline-flex items-center gap-1.5 text-[10px] font-black text-blue-400 bg-blue-500/10 border border-blue-500/25 px-2.5 py-1 rounded-full uppercase tracking-widest mb-2">
                  <FacebookLogo size={9} /> Facebook
                </span>
                <h2 className="text-2xl sm:text-3xl font-black text-white leading-tight tracking-tight">AutomateQA</h2>
                <p className="text-[#6B7280] text-sm font-mono mt-0.5">Automate QA</p>
                <p className="text-[#9CA3AF] text-sm mt-2 leading-relaxed hidden sm:block max-w-xs">
                  Join our QA community — tutorials, discussions &amp; the latest automation news.
                </p>
              </div>
            </div>
            ...
            <div className="sm:ml-auto flex items-center gap-3 flex-wrap">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-sm font-black transition-all duration-200 hover:shadow-[0_0_28px_rgba(59,130,246,0.5)] active:scale-95"
              >
                <Bell size={14} className="fill-white stroke-none" />
                Like Page
              </a>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-6 border-t border-white/[0.06]">
            {facebookContent.map(({ emoji, title, desc }) => (
              <div key={title} className="flex items-start gap-3.5 p-4 rounded-2xl bg-white/[0.025] border border-white/[0.06] hover:border-blue-500/25 hover:bg-blue-500/[0.04] transition-all duration-300 group cursor-default">
                <span className="text-xl mt-0.5 flex-shrink-0">{emoji}</span>
                <div>
                  <div className="text-sm font-black text-white group-hover:text-blue-300 transition-colors mb-0.5">{title}</div>
                  <div className="text-xs text-[#6B7280] leading-relaxed">{desc}</div>
                </div>
              </div>
            ))}
