...
export default function TipInteractions({
  tipId,
  initialLikes = 0,
  title,
}: {
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem(`tip_liked_${tipId}`)) {
      setLiked(true);
  ...
  async function handleLike() {
    ...
    setLiked(true);
    setLikesCount((c) => c + 1);
    localStorage.setItem(`tip_liked_${tipId}`, "1");
    try {
      const { data } = await supabase.rpc("increment_tip_likes", { tip_id_param: tipId });
  ...
  async function handleComment(e: React.FormEvent) {
    ...
    try {
      ...
      setCommentError("Failed to post. Make sure the tip_comments table exists in Supabase.");
    }
    setSubmitting(false);
  }
            ...
            <form onSubmit={handleComment} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Share your thoughts, questions, or extra tips! 💡"
                  maxLength={2000}
                  rows={3}
                  required
